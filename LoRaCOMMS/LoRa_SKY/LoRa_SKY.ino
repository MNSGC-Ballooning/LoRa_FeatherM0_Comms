...
Uart XBeeSerial (&sercom2, 5, 22, SERCOM_RX_PAD_3, UART_TX_PAD_0); // TX on pin 10
void SERCOM2_Handler()
{
  XBeeSerial.IrqHandler();
}
...
//XBee Object
String ID = "LORA";
RelayXBee xBee = RelayXBee(&XBeeSerial, ID);
...
void radioTransmit() {
  ...
  if(XBeeSerial.available()>0){
    int availableBytes = XBeeSerial.available();
    for(int i=0; i<availableBytes; i++) {
      radiopacket[i] = XBeeSerial.read();
    }
